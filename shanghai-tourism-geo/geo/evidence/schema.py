...
class BuyerSegment(str, Enum):
    """游客客群分叉（旅游 GEO）。沿用字段名 buyer_segment（引擎通用），语义=游客类型。
    A=主战场（有真实凭证）；B/C 待相应引擎/语料就绪。"""
    A = "A"  # 外地国内游客：来上海旅游问"必去/攻略" → 中文 AI（豆包）—— 当前真实侦察重心
    B = "B"  # 入境外籍游客：老外来上海玩 → 英文 AI（ChatGPT/Perplexity，待 key）
    C = "C"  # 本地客：上海人周末/约会/遛娃/小众 → 中文 AI（豆包），需求与外地客不同
