...
class ReconQuery:
    ...
    intent: str = "recommend"  # recommend 景点推荐 / plan 行程规划 / howto 攻略 / compare 对比 / decide 决策
    theme: str = ""
...
SHANGHAI_QUERIES: list[ReconQuery] = [
...
def queries_for_segment(segment: BuyerSegment) -> list[ReconQuery]:
    return [q for q in SHANGHAI_QUERIES if q.buyer_segment == segment]


def phase0_slice() -> dict[str, ReconQuery]:
    """最小切片：1 条外地客（豆包真跑）+ 1 条入境客（英文）。"""
    a = next(q for q in SHANGHAI_QUERIES if q.buyer_segment == BuyerSegment.A)
    b = next(q for q in SHANGHAI_QUERIES if q.buyer_segment == BuyerSegment.B)
    return {"zh": a, "en": b}
