...
GIFT_BOX_QUERIES: list[ReconQuery] = [
...
def queries_for_segment(segment: BuyerSegment) -> list[ReconQuery]:
    return [q for q in GIFT_BOX_QUERIES if q.buyer_segment == segment]


def phase0_slice() -> dict[str, ReconQuery]:
    """Phase 0 最小切片：1 条中文（豆包真跑）+ 1 条英文（mock 占位）。"""
    zh = next(q for q in GIFT_BOX_QUERIES if q.buyer_segment == BuyerSegment.A)
    en = next(q for q in GIFT_BOX_QUERIES if q.buyer_segment == BuyerSegment.B)
    return {"zh": zh, "en": en}
