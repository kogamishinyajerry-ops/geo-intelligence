...
_CONTENT_DRAFTS_DIR = REPO_ROOT / "content" / "drafts"
_MONITORING_HISTORY_DIR = REPO_ROOT / "monitoring" / "history"
...
def _build_monitoring() -> dict:
    history_dir = _MONITORING_HISTORY_DIR
...
def _build_content_pipeline(opportunity: list[dict]) -> list[dict]:
    ...
    if not _CONTENT_DRAFTS_DIR.exists():
        return []
    ...
    out = []
    for path in sorted(_CONTENT_DRAFTS_DIR.glob("*.md")):
