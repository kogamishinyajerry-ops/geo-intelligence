...
def _save_and_report(store, cap, label: str, repo_root) -> None:
    path = store.save(cap)
...
def run_phase0() -> int:
    ...
    zh = sl["zh"]
    _save_and_report(store, doubao.run(zh.text, zh.buyer_segment), "豆包·真实", settings.category_root)

    # 英文：mock 占位（明确标注非真证据）
    en = sl["en"]
    mock = MockAdapter(
        store, watchlist, min_interval_sec=0, repo_root=settings.category_root, engine_label="mock-perplexity"
    )
    _save_and_report(store, mock.run(en.text, en.buyer_segment), "mock·占位", settings.category_root)
    return 0
...
def run_mock_only() -> int:
    ...
    for key, label in (("zh", "mock-doubao"), ("en", "mock-perplexity")):
        q = sl[key]
        mock = MockAdapter(
            store, watchlist, min_interval_sec=0, repo_root=settings.category_root, engine_label=label
        )
        _save_and_report(store, mock.run(q.text, q.buyer_segment), label, settings.category_root)
    return 0
