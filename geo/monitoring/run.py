...
def _mon_paths():
    """监测输出目录：按 active 品类的 category_root 运行时解析（合并后 monitoring/ 在 categories/<cat>/ 下）。"""
    root = get_settings().category_root
    mon = root / "monitoring"
    return root, mon / "alerts", mon / "notion_queue", mon / "ALERTS.md"
...
def run(
    ...
    # 告警落地（truth plane）—— 目录按 active 品类 category_root 运行时解析
    root, ALERTS_DIR, QUEUE_DIR, ROLLING_LOG = _mon_paths()
    ...
    print(f"快照 → {snap_path.relative_to(root)}")
    print(format_alerts(diff))
    print(f"告警 → {(ALERTS_DIR / f'{segment.value}-{stamp}.md').relative_to(root)}")
    print(f"Notion 队列 → {(QUEUE_DIR / f'{segment.value}-{stamp}.json').relative_to(root)}")
    return diff
